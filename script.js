...
function getSpinPrizes() {
  const raw = (Array.isArray(appSettings.spin_prizes) && appSettings.spin_prizes.length)
    ? appSettings.spin_prizes
    : SPIN_PRIZES_FALLBACK;
  // GRAM has been removed from the spin wheel - strip any old GRAM rows still
  // saved in settings.spin_prizes so they stop showing on the wheel visually
  // (the backend already never awards them, this just keeps the UI honest).
  const filtered = raw.filter(p => p.type !== 'GRAM');
  return filtered.length ? filtered : SPIN_PRIZES_FALLBACK;
}
