...
// =====================================================
// DEVICE VERIFICATION — one Telegram account per device
// =====================================================
// Generates a random ID the first time the app opens on this device/app-instance
// and keeps it in localStorage forever after. Sent with /api/user/sync so the
// backend can refuse to create/login a SECOND Telegram account from a device
// that's already linked to a different one. This is a deterrent, not a perfect
// lock — clearing Telegram's app storage / reinstalling resets it — but it
// blocks the casual multi-accounting that farms daily/referral/ad rewards.
...
function getDeviceId() {
  try {
    ...
    // per-session id so the app still works, it just won't persist device
    // identity across app restarts.
    return 'nostorage-' + Date.now() + '-' + Math.random().toString(36).slice(2);
  }
}
let deviceBlocked = false; // set true if the backend refuses this device

// Hardware-based fingerprint - does NOT rely on localStorage, so it survives
// Telegram's "Add Account" switch (which gives the Mini App a fresh, isolated
// localStorage per account) and cache clears / reinstalls. Built from things
// tied to the actual physical device (GPU rendering, screen, hardware specs)
// rather than anything saved on disk. Not perfect: two people with the exact
// same phone model can occasionally produce the same fingerprint - a false
// positive, not a false negative, so it errs toward blocking rather than
// letting real multi-accounters through.
function getDeviceFingerprint() {
  try {
    ...
    // NOTE: timeZone and language deliberately excluded from this fingerprint.
    // On devices with "automatic (network-provided)" time zone / region,
    // toggling flight mode can briefly change the detected zone/locale,
    // which flipped the entire fingerprint hash and let multi-accounters
    // bypass the device lock just by turning flight mode on/off. Only
    // genuinely hardware-tied signals are kept below.
    ...
    try {
      ...
      ctx.fillStyle = '#069';
      ctx.fillText('MilkyRush-device-check', 2, 2);
    ...
    // Plain djb2 hash - this only needs to be a stable fingerprint, not
...
let appSettings = { id: 1, ad_reward: 50, ads_daily_limit: 10, referral_instant_reward: 50, referral_ads_reward: 100, min_withdrawal_gram: 0.1, min_withdrawal_usdt: 0.1, min_swap_gram_milk: 10, min_swap_usdt_milk: 10, daily_reward_base: 100, channel_reward: 1000, tap_energy_max: 100, tap_cooldown_minutes: 30, tap_daily_limit: 500, withdrawal_ads_required: 10, withdrawal_ads_enabled: true, withdrawal_referrals_required: 3, withdrawal_referrals_enabled: true, spin_ads_required: 5, spin_prizes: null };
let allUsersCache = [];
  ...
  try {
    ...
    if (rows.length > 0) {
      appSettings = {
        ...
        min_withdrawal_gram: rows[0].min_withdrawal_gram ?? 0.1,
        min_withdrawal_usdt: rows[0].min_withdrawal_usdt ?? 0.1,
        min_swap_gram_milk: rows[0].min_swap_gram_milk ?? 10,
...
async function saveSettings() {
  ...
  const minWithdrawalGram = parseFloat(document.getElementById('settingMinWithdrawalGram').value) || 0.1;
  const minWithdrawalUsdt = parseFloat(document.getElementById('settingMinWithdrawalUsdt').value) || 0.1;
  const minSwapGram = parseInt(document.getElementById('settingMinSwapGram').value) || 10;
  ...
  try {
    await adminApi('/api/admin/settings', {
      ...
      min_withdrawal_gram: minWithdrawalGram,
      min_withdrawal_usdt: minWithdrawalUsdt,
      min_swap_gram_milk: minSwapGram,
...
window.addEventListener('load', async () => {
  ...
  if (!deviceBlocked) await loadChannels();
  setTimeout(() => { document.getElementById('loadingScreen').style.display='none'; }, 2000);

  if (deviceBlocked) return; // this device is already linked to another account — stop here
...
async function loadOrCreateUser() {
  try {
    ...
    const { user, created } = await apiUser('/api/user/sync', { ref: refCode || null, device_id: getDeviceId(), device_fp: getDeviceFingerprint() });
    currentUser = user;
    ...
    // Backend sends this specific error code when this device is already
    // linked to a different Telegram account (see requireDeviceCheck in
    // index.js) — show the dedicated block screen instead of the generic
    // connection-error toast.
    if (e.message === 'device_already_registered') {
      deviceBlocked = true;
      document.getElementById('userApp').style.display = 'none';
      document.getElementById('deviceBlockedScreen').classList.add('active');
    } else {
...
async function renderUI() {
  ...
  const gramBal = (currentUser.gram_balance || 0);
  const usdtBal = (currentUser.usdt_balance || 0);
  document.getElementById('gramBalance').textContent = gramBal.toLocaleString(undefined, {maximumFractionDigits:4});
  document.getElementById('usdtBalance').textContent = usdtBal.toLocaleString(undefined, {maximumFractionDigits:4});
  document.getElementById('gramBalanceSub').textContent = `${gramBal.toLocaleString(undefined, {maximumFractionDigits:4})} GRAM`;
...
// Config for each withdrawal method: unit label, address field label/placeholder, and min chars for the identifier.
const WITHDRAW_METHODS = {
  GRAM: {
    unit: 'GRAM',
    balanceField: 'gram_balance',
    label: '💎 Your TON Wallet Address (GRAM)',
  ...
  BINANCE_UID: {
    unit: 'USDT',
    balanceField: 'usdt_balance',
    label: '🅱️ Your Binance UID',
    placeholder: 'Enter your Binance UID (Pay ID)',
    minLen: 5,
  },
};

function getMinWithdrawal(method) {
  const unit = WITHDRAW_METHODS[method].unit;
  return unit === 'GRAM'
    ? (appSettings.min_withdrawal_gram ?? 0.1)
    : (appSettings.min_withdrawal_usdt ?? 0.1);
}

let withdrawMethod = 'GRAM';

function selectWithdrawMethod(method) {
  withdrawMethod = method;
  document.querySelectorAll('#withdrawMethodTabs .method-tab').forEach((btn) => {
    const active = btn.dataset.method === method;
    btn.classList.toggle('active', active);
    btn.style.background = active ? 'var(--accent)' : 'transparent';
    btn.style.color = active ? '#181029' : 'var(--fg)';
  });
  const cfg = WITHDRAW_METHODS[method];
  document.getElementById('gramAddressLabel').textContent = cfg.label;
  document.getElementById('gramAddress').placeholder = cfg.placeholder;
  document.getElementById('gramAddress').value = '';
  // Switching method can switch the underlying balance (GRAM vs USDT) and its
  // own admin-configured minimum, so reset the amount to that minimum.
  exchangeAmount = Math.min(getMinWithdrawal(method), currentUser?.[cfg.balanceField] || 0) || getMinWithdrawal(method);
  renderQuickAmountButtons(getMinWithdrawal(method));
  refreshExchangeUI();
}
...
function openWithdrawModal(method) {
  ...
  const initialMethod = method || 'GRAM';
  const unit = WITHDRAW_METHODS[initialMethod].unit;
  // Only show the tab(s) matching the currency the user tapped into from the
  // Wallet screen: GRAM row -> only GRAM tab; USDT row -> BEP20 + Binance UID
  // tabs (GRAM hidden), since those two both draw from the same usdt_balance.
  document.querySelectorAll('#withdrawMethodTabs .method-tab').forEach((btn) => {
    btn.style.display = btn.dataset.unit === unit ? '' : 'none';
  });
  selectWithdrawMethod(initialMethod);
...
// withdrawal for the currently selected method (GRAM or USDT), shown as
// decimals since these are now unit amounts, not Milk.
...
function setExchangeAmount(val) {
  const cfg = WITHDRAW_METHODS[withdrawMethod];
  if (val === 'max') {
    exchangeAmount = currentUser?.[cfg.balanceField] || 0;
  } else {
    exchangeAmount = val;
  }
  refreshExchangeUI();
...
function refreshExchangeUI(skipAmountFieldUpdate) {
  ...
  const cfg = WITHDRAW_METHODS[withdrawMethod];
  const minWithdrawal = getMinWithdrawal(withdrawMethod);
...
// ===== MILK SWAP (internal conversion to GRAM / USDT balance) =====

let swapTarget = 'GRAM';
let swapAmount = MIN_SWAP_MILK;

function getMinSwapMilk() {
  return swapTarget === 'GRAM'
    ? (appSettings.min_swap_gram_milk ?? MIN_SWAP_MILK)
    : (appSettings.min_swap_usdt_milk ?? MIN_SWAP_MILK);
}

function selectSwapTarget(target) {
  swapTarget = target;
  const icon = document.getElementById('swapTargetIcon');
  const label = document.getElementById('swapTargetLabel');
  if (icon) icon.className = `token-icon ${target === 'GRAM' ? 'icon-gram' : 'icon-usdt'}`;
  if (label) label.textContent = target;
  refreshSwapUI();
}

function toggleSwapTarget() {
  selectSwapTarget(swapTarget === 'GRAM' ? 'USDT' : 'GRAM');
}
...
function refreshSwapUI(skipAmountFieldUpdate) {
  if (!currentUser) return;
  const rate = swapTarget === 'GRAM' ? MILK_PER_GRAM : MILK_PER_USDT;
  const unit = swapTarget;
  ...
  document.getElementById('swapRateInfoLabel').textContent = `100,000 Milk = 1 ${unit}`;
...
async function confirmSwap() {
  ...
  try {
    // Server re-checks balance and performs the Milk debit + GRAM/USDT credit
    ...
    await renderUI();
    const rate = swapTarget === 'GRAM' ? MILK_PER_GRAM : MILK_PER_USDT;
    showToast(`✅ Swapped! +${(swapAmount / rate).toFixed(4)} ${swapTarget}`);
  } catch (e) {
...
const SPIN_PRIZES_FALLBACK = [
  ...
  { type: 'GRAM', amount: 0.01, weight: 15 },
  { type: 'USDT', amount: 0.01, weight: 15 },
];
  ...
  return `<div class="spin-prize-row" style="display:flex;flex-direction:column;gap:6px;margin-bottom:10px;padding:8px;border:1px solid var(--border);border-radius:10px;" data-idx="${i}">
    <div style="display:flex;gap:6px;align-items:center;">
      <select class="spin-prize-type" style="flex:1;padding:8px;border-radius:8px;border:1px solid var(--border);background:#181029;color:var(--fg);font-family:inherit;">
        ...
        <option value="GRAM" ${p.type === 'GRAM' ? 'selected' : ''}>💎 GRAM</option>
...
function renderSpinWheel() {
  ...
  // GRAM/USDT use the SAME real icon images as the Wallet screen (not emoji,
  // which don't match the actual TON/Tether branding); Milk keeps the emoji
...
async function loadAdminData() {
  try {
    ...
    document.getElementById('settingMinWithdrawalGram').value = appSettings.min_withdrawal_gram;
    document.getElementById('settingMinWithdrawalUsdt').value = appSettings.min_withdrawal_usdt;
    document.getElementById('settingMinSwapGram').value = appSettings.min_swap_gram_milk;
...
function renderUsersTable() {
  ...
  filtered.forEach(u => {
    ...
    const gramBal = u.gram_balance || 0;
    const usdtBal = u.usdt_balance || 0;
    userHTML += `<tr>
      <td>${u.id}</td>
      <td>${escapeHtml(u.first_name)}</td>
      <td>${(u.balance || 0).toLocaleString()}</td>
      <td>${gramBal.toLocaleString(undefined, {maximumFractionDigits:4})}</td>
      ...
      <td>
        <div class="row-actions">
          ...
          <button class="btn-mini-blue" onclick="openAdjustBalance(${u.id}, '${escapeHtml((u.first_name||'').replace(/'/g,""))}', ${u.balance || 0}, ${gramBal}, ${usdtBal})">💰 Add/Sub</button>
  ...
  if (filtered.length === 0) {
    userHTML = '<tr><td colspan="10" style="text-align:center;color:var(--muted);">No users found</td></tr>';
  }
...
// network round-trip.
let adjustBalanceBalances = { milk: 0, gram: 0, usdt: 0 };
let adjustBalanceName = '';

function openAdjustBalance(userId, name, balance, gramBalance, usdtBalance) {
  adjustBalanceTargetId = userId;
  adjustBalanceName = name || 'User';
  adjustBalanceBalances = { milk: balance || 0, gram: gramBalance || 0, usdt: usdtBalance || 0 };
...
// precision (Milk is whole numbers, GRAM/USDT carry decimals since they're
// converted at MILK_PER_GRAM rate).
...
async function submitAdjustBalance(direction) {
  ...
  const currency = document.getElementById('adjustBalanceCurrency').value;
  // Milk is whole-number only; GRAM/USDT allow decimals (they're stored as
  // fractional amounts, e.g. 0.25 GRAM).
